...
CERT-005 Certificate Verification adds **no migration**. It reuses the `verification_id` minted in CERT-003 and reads through a narrow privileged server-side lookup rather than a public policy: no `to anon` or `to public` RLS policy exists on any certificate table, and none is added. The public lookup is exact equality on `verification_id` only — no prefix, pattern, ordering or listing — and it never selects `user_id`, never queries `public.user_profiles`, and never reads any Evidence table. Lifecycle status is derived through CERT-004's resolver and fails closed as temporarily unavailable rather than reporting a genuine certificate as invalid.
